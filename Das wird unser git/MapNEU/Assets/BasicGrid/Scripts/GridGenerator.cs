...
public class GridGenerator : MonoBehaviour
{
		...
		void CreateUnit (int i)
		{
				...
				case Orientation.horizontal:
						newPosition = new Vector3 (_positions [i, 0], spawnObject.transform.position.y, _positions [i, 1]);
						break;
				}
				...
				for (int i = 0; i < 4; i++) {
						...
						if (i % 2 == 0) {
								...
								if (orientation == Orientation.horizontal) {
										...
										bounderie.transform.localScale = new Vector3 (_bounderieWidth, bounderieHeight, 0);
								}

								if (orientation == Orientation.vertical) {
										bounderie.transform.position = new Vector3 (0, _distFromMid, -bounderieHeight / 2);
										bounderie.transform.localScale = new Vector3 (_bounderieWidth, 0, bounderieHeight);
								}
						}
						...
						if (i % 2 == 1) {
								...
								if (orientation == Orientation.horizontal) {
										bounderie.transform.localScale = new Vector3 (0, bounderieHeight, _bounderieLength);
										bounderie.transform.position = new Vector3 (_distFromMid, bounderieHeight / 2, 0);
								}

								if (orientation == Orientation.vertical) {
										bounderie.transform.localScale = new Vector3 (0, _bounderieLength, bounderieHeight);
		...
		void CreateBackground ()
		{
				...
				// Add the assigned material to the background
				if (backgroundMaterial != null) {
						background.GetComponent<MeshRenderer> ().material = backgroundMaterial;
				} else
						Debug.LogWarning ("The material was not assigned", backgroundMaterial);
		}
